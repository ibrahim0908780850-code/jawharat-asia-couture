...
import { BrandLogo, CollectionCard, GalleryTile, LineArrow, PillarIcon, ProcessStep, SectionHeading } from "@/components/atelier";
    ...
    <div className="site-shell" id="top">
      ...
      <header className={`site-header ${scrolled ? "site-header--scrolled" : ""}`}>
        <div className="header-inner">
          <a href="#top" className="brand-lockup" aria-label={siteConfig.brandName} onClick={() => setMenuOpen(false)}><BrandLogo /></a>
      ...
      <footer className="site-footer"><div className="container"><div className="footer-top"><a href="#top" className="brand-lockup brand-lockup--footer"><BrandLogo /></a><p>نصمم اللحظة التي<br />تشبهكِ تمامًا.</p><a className="footer-backtop" href="#top">العودة للأعلى <MoveUpLeft size={16} /></a></div><div className="footer-grid"><div><span className="footer-label">استكشفي</span>{navigationItems.slice(0, 4).map((item) => <a key={item.href} href={item.href}>{item.label}</a>)}</div><div><span className="footer-label">تواصلي</span><a href="#appointment">حجز موعد</a><a href={siteConfig.socialLinks.tiktok} target="_blank" rel="noreferrer">TikTok</a><a href={siteConfig.socialLinks.snapchat} target="_blank" rel="noreferrer">Snapchat</a></div><div><span className="footer-label">الموقع</span><a href={siteConfig.mapUrl} target="_blank" rel="noreferrer">الرياض، السعودية</a></div><div className="footer-monogram"><BrandLogo compact /><span>ASIA</span></div></div><div className="footer-bottom"><span>© 2026 {siteConfig.brandName}</span><span>جميع الحقوق محفوظة.</span><span>صُنع بحب في الرياض</span></div></div></footer>
