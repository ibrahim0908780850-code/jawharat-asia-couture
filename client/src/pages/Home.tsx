  ...
  return (
    <div className="site-shell" id="top">
      ...
      <main id="main-content">
        ...
        <section className="appointment-form section-dark" id="appointment-form"><div className="container"><div className="form-layout"><div className="form-intro"><p className="eyebrow"><span className="eyebrow-line" />خطوتك الأولى</p><h2>لنبدأ<br /><em>من فكرتك.</em></h2><p>أرسلي لنا تفاصيل بسيطة عن موعدك، وسنعود إليكِ لتنسيق الاستشارة.</p><div className="form-note"><Clock3 size={17} /><span>الرد يتم خلال أوقات العمل<br /><small>الرياض · بمواعيد مسبقة</small></span></div></div>{formSent ? <div className="form-success"><span><Check size={27} /></span><h3>تم فتح واتساب.</h3><p>تم تجهيز رسالة الحجز وإرسالها إلى محادثة الرقم 0500144730.</p><button className="text-link" onClick={() => setFormSent(false)}>إرسال طلب آخر <ArrowLeft size={17} /></button></div> : <form className="booking-form" onSubmit={handleBookingSubmit}><label>الاسم الكامل<input required name="name" placeholder="اكتبي اسمك هنا" /></label><label>طريقة التواصل<input required name="contact" placeholder="رقم الجوال أو البريد الإلكتروني" /></label><label>ما المناسبة؟<select required name="occasion" defaultValue=""><option value="" disabled>اختاري المناسبة</option><option>فستان زفاف</option><option>فستان سهرة</option><option>تصميم حسب الطلب</option></select></label><button className="button button--gold button--full" type="submit">إرسال الطلب عبر WhatsApp <Send size={16} /></button><small className="form-privacy">بإرسال الطلب، سيتم فتح WhatsApp برسالة تحتوي على بيانات الحجز.</small></form>}</div></div></section>
      </main>
